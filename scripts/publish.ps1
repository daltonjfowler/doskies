...
# Publish the current release build to the DOSkies update host. Run from the repo
# root AFTER a green scripts/build.ps1, which produces releases/DOSkies.apk and
...
$apk = Join-Path $repoRoot 'releases/DOSkies.apk'
...
if (-not (Test-Path -LiteralPath $apk)) { throw "No release APK at releases/DOSkies.apk. Run scripts/build.ps1 first." }
...
if ($actual -ne $recorded) {
    throw "releases/SHA256SUMS.txt ($recorded) does not match releases/DOSkies.apk ($actual). Re-run scripts/build.ps1."
}
...
try {
    $live = Invoke-RestMethod -Uri 'https://doskies-updates.daltonjfowler.workers.dev/latest.json' -Headers @{ 'Cache-Control' = 'no-cache' } -TimeoutSec 15
...
Copy-Item -LiteralPath $apk -Destination (Join-Path $publicDir 'DOSkies.apk') -Force
...
$json = '{"version_code": ' + $versionCode + ', "version_name": "' + $versionName + '", "sha256": "' + $actual + '", "size": ' + $size + ', "apk": "DOSkies.apk", "notes": "' + $notesEscaped + '"}'
...
if ($NoDeploy) {
    Write-Output 'NoDeploy set: stopping before wrangler. latest.json and DOSkies.apk are staged in updates/public.'
    return
}
...
for ($i = 0; $i -lt 6 -and -not $confirmed; $i++) {
    ...
    try {
        $served = Invoke-RestMethod -Uri 'https://doskies-updates.daltonjfowler.workers.dev/latest.json' -Headers @{ 'Cache-Control' = 'no-cache' } -TimeoutSec 20
...
else { Write-Warning "Deployed, but live latest.json has not shown version_code $versionCode yet. Check https://doskies-updates.daltonjfowler.workers.dev/latest.json in a moment." }
