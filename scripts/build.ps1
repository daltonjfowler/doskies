...
try {
    ...
    Copy-Item -LiteralPath ./app/build/outputs/apk/release/app-release.apk -Destination ./releases/DOSkies.apk -Force
    $hash = (Get-FileHash -LiteralPath ./releases/DOSkies.apk -Algorithm SHA256).Hash.ToUpperInvariant()
    "$hash  DOSkies.apk" | Out-File -FilePath ./releases/SHA256SUMS.txt -Encoding ascii -NoNewline
    Write-Output "Built releases/DOSkies.apk  sha256=$hash"
    if ($Install) {
        & "$env:ANDROID_HOME/platform-tools/adb.exe" install -r ./releases/DOSkies.apk
